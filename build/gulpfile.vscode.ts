...
function packageTask(platform: string, arch: string, sourceFolderName: string, destinationFolderName: string, _opts?: { stats?: boolean }) {
	...
	const task = () => {
		...
		// ThirdPartyNotices-Extensions.txt carries the license and copyright notice of every extension in
		// product.json's builtInExtensions. Those are redistributed in the installer, and notice-only
		// licenses require their notice to travel with the binary, so it has to be packaged here -- a
		// notices file that stays in the source tree discharges nothing. Generated by
		// build/nexgile/syncLanguageExtensions.ts; allowEmpty covers a tree where it has not been run yet.
		const license = gulp.src(
			[product.licenseFileName, 'ThirdPartyNotices.txt', 'ThirdPartyNotices-Extensions.txt', 'licenses/**'],
