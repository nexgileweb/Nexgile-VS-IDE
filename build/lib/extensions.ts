...
export function packageCopilotExtensionStream(_disableMangle: boolean): Stream {
	// Nexgile: never ship VS Code's built-in GitHub Copilot extension. It activates on
	// startup (onStartupFinished) and registers the github.copilot.default chat participant
	// plus language-model providers, which would re-enable VS Code's native chat UI even
	// when chat.disableAIFeatures is set. It also bundles multi-platform native binaries
	// (@anthropic-ai/claude-agent-sdk) that break the win32 rcedit version-stamping step.
	// All AI in this product is provided by the bundled nexgile-code extension, so the
	// upstream packaging of extensions/copilot is intentionally skipped entirely.
	return es.readArray([]);
}
